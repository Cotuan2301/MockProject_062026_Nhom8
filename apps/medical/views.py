...
def initial_assessment(request, pk, assessment_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            obj.assessed_by = request.user if request.user.is_authenticated else None
            ...
            return redirect('medical:loc_detail', assessment_id=obj.id)
        else:
...
def loc_classification_detail(request, assessment_id):
    ...
    return render(request, 'medical/sc_024-loc_classification.html', context)
...
def loc_classification_confirm(request, assessment_id):
    if request.method == 'POST':
        ...
        if loc.status in ['CONFIRMED', 'Confirmed']:
...
def create_reassessment_api(request, resident_id):
    ...
    new_assessment = Assessment.objects.create(
        ...
        care_level=latest_assessment.care_level if latest_assessment else '',
        is_locked=False
    )
    
    return redirect('medical:initial_assessment', pk=resident.pk, assessment_id=new_assessment.id)
