...
class ResidentCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        address_data = {k: validated_data.pop(k, None) for k in ['address_line1', 'address_line2', 'address_city', 'address_state', 'address_zip_code']}
        contacts_data = {k: validated_data.pop(k, None) for k in ['phone_primary', 'phone_secondary', 'emergency_first_name', 'emergency_last_name', 'emergency_phone_primary', 'emergency_phone_secondary', 'poa_first_name', 'poa_last_name', 'poa_phone_primary', 'poa_phone_secondary', 'poa_relationship']}
        insurance_data = {k: validated_data.pop(k, None) for k in ['insurance_provider_name', 'insurance_provider_type', 'policy_number', 'policy_effective_from', 'policy_effective_to', 'auth_number']}
        ...
        self._handle_address(resident, address_data)
        self._handle_contacts(resident, contacts_data)
        self._handle_insurance(resident, insurance_data)
    ...
    def update(self, instance, validated_data):
        ...
        self._handle_address(instance, address_data)
        self._handle_contacts(instance, contacts_data)
        self._handle_insurance(instance, insurance_data)
        
        return instance

    def _handle_address(self, resident, address_data):
    ...
    def _handle_contacts(self, resident, contacts_data):
        phone_primary = contacts_data.get('phone_primary')
        phone_secondary = contacts_data.get('phone_secondary')
        emergency_first_name = contacts_data.get('emergency_first_name')
        emergency_last_name = contacts_data.get('emergency_last_name')
        emergency_phone_primary = contacts_data.get('emergency_phone_primary')
        emergency_phone_secondary = contacts_data.get('emergency_phone_secondary')
        poa_first_name = contacts_data.get('poa_first_name')
        poa_last_name = contacts_data.get('poa_last_name')
        poa_phone_primary = contacts_data.get('poa_phone_primary')
        poa_phone_secondary = contacts_data.get('poa_phone_secondary')
        poa_relationship = contacts_data.get('poa_relationship')
    ...
    def _handle_insurance(self, resident, insurance_data):
        insurance_provider_name = insurance_data.get('insurance_provider_name')
        insurance_provider_type = insurance_data.get('insurance_provider_type')
        policy_number = insurance_data.get('policy_number')
        policy_effective_from = insurance_data.get('policy_effective_from')
        policy_effective_to = insurance_data.get('policy_effective_to')
        auth_number = insurance_data.get('auth_number')
        ...
        if insurance_provider_name or policy_number or policy_effective_from or policy_effective_to:
            ...
            if policy:
                ...
                if auth_number is not None:
                    policy.auth_number = auth_number
                policy.save()
            elif provider:
